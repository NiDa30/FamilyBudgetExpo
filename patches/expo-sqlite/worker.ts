...
async function handleMessageImpl<T extends SQLiteWorkerMessageType>({
  ...
  switch (type) {
    ...
    case "importAssetDatabase": {
      await importAssetDatabase(
        data.databasePath,
        data.assetDatabasePath,
        data.forceOverwrite
      );
      break;
    }
    ...
    case "sessionCreateChangeset": {
      result = await sessionCreateChangeset(
        data.nativeDatabaseId,
        data.nativeSessionId
      );
      break;
    ...
    case "sessionCreateInvertedChangeset": {
      result = await sessionCreateInvertedChangeset(
        data.nativeDatabaseId,
        data.nativeSessionId
      );
      break;
...
async function backupDatabase(
  ...
  if (!destDb)
    throw new Error(
      `Database not found - nativeDatabaseId[${destNativeDatabaseId}]`
    );
  const sourceDb = databaseIdMap.get(sourceNativeDatabaseId);
  if (!sourceDb)
    throw new Error(
      `Database not found - nativeDatabaseId[${sourceNativeDatabaseId}]`
    );
  await sqlite3.backup(
    destDb.pointer,
    destDatabaseName,
    sourceDb.pointer,
    sourceDatabaseName
...
async function exec(nativeDatabaseId: number, source: string) {
  ...
  if (!dbEntity)
    throw new Error(
      `Database not found - nativeDatabaseId[${nativeDatabaseId}]`
    );
...
async function finalize(
  ...
  if (!stmt)
    throw new Error(
      `Statement not found - nativeStatementId[${nativeStatementId}]`
    );
...
async function importAssetDatabase(
  ...
  if (!forceOverwrite) {
    ...
    await vfs.jAccess(
      databasePath,
      0 /* unused arg for AccessHandlePoolVFS */,
      buffer
    );
...
async function openDatabase(
  ...
  if (serializedData) {
    ...
    const vfsName =
      databasePath === ":memory:" ? VFS_NAME_MEMORY : VFS_NAME_PERSISTENT;
...
async function run(
  ...
  const firstRowValues =
    ret === SQLITE_ROW ? getColumnValues(sqlite3, stmt.pointer) : [];
  return {
...
async function sessionAttach(
  ...
  if (!session)
    throw new Error(`Session not found - nativeSessionId[${nativeSessionId}]`);
  ...
  sqlite3.update_hook(
    ...
    (
      ...
      const message: OnDatabaseChangeMessage = {
        ...
        data: {
          databaseName: dbName,
          databaseFilePath: sqlite3.db_filename(
            dbEntity.pointer,
            dbName ?? "main"
          ),
          tableName: tblName,
...
function findCachedDatabase(
  predicate: (entity: DatabaseEntity) => boolean
): DatabaseEntity | null {
...
async function maybeFinalizeAllStatements(nativeDatabaseId: number) {
  ...
  while (stmt != null && stmt !== 0) {
    const nextStmt: StatementPointer = sqlite3.next_stmt(
      dbEntity.pointer,
      stmt
    );
    try {
